...
kubectl get configmap postgresconfigmap --namespace=$sharedNamespace -o yaml | grep -v '^\s*namespace:\s' | kubectl apply --namespace=$envId -f -

### Copy Secrets ###
kubectl get secret postgressecret --namespace=$sharedNamespace -o yaml | grep -v '^\s*namespace:\s' | kubectl apply --namespace=$envId -f -
